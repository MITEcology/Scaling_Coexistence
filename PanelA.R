...
interaction_matrix_random <- function(M,num,alpha,E){
  ...
  for (a in 1:length(M)) {
    for (b in 1:length(M)) {
      Inte[a,b] <- (M[b])^(alpha)/(M[a])^(alpha)*exp((E[a]-E[b]))*(1/sqrt(num))
    }
  }
...
for (i in 1:length(rbeta)) {
  ...
  angle[i] <- theta(-A,r_mt) ### DISTANCE
}


### PLOT
plot(rbeta,angle,xlab ='Beta',xaxt = "n",ylab = 'Average distance',type='o',col='dodgerblue',cex.lab=1,lwd=1.5,axis.ticks=3,ylim = c(0,90))
